...
This is a binary-to-thermoeter encoder implemented in SystemVerilog.  This circuit is combinational.  The bit width of the input _bin_ can be spefified by the parameter _WIDTH_.  The bit width of the output _out_ is 2^_WIDTH_^-1.  When _bin_ == 0, the all bits of _out_ become 0.  When _bin_ == 2^_WIDTH_^-1, the all bits of _out_ become 1.
